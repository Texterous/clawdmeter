...
OtaLatest otaCheckLatest(const Settings& s) {
  ...
  for (int attempt = 1; attempt <= kAttempts; attempt++) {
    ...
    SecureClient client;
    client.setInsecure();
#if 1   // ESP8266: MFLN probe keeps the BearSSL buffer small
    ...
    // at boot, before the meter claims heap) rather than blocking long on each try.
    ...
    // getStream(), which the core does not de-chunk.
    http.useHTTP10(true);
...
#else   // !WITH_SELFUPDATE — the slim image has no TLS stack at all

// Stubs for the slim build. It cannot reach GitHub, so the System page hides
// the button (see handleGetConfig) and these only exist to keep the link happy.
OtaLatest otaCheckLatest(const Settings&) {
  OtaLatest r;
  r.error = F("this build has no TLS; upload the firmware in the System tab");
  return r;
}
